...
from spurious_corr.utils import pretty_print, pretty_print_dataset, highlight_from_file, highlight_from_list, highlight_html, highlight_dates
...
def example_1_basic_date_injection():
    ...
    modifier_start = ItemInjection.from_function(
        injection_func=date_gen,
        location="beginning", 
        token_proportion=0.4,
        seed=42
    )
...
def example_2_file_based_injection():
    ...
    custom_highlighter = highlight_from_list(["URGENT", "BREAKING", "EXCLUSIVE", "ALERT"])
...
def example_4_multiple_injections():
    ...
    # Combine all
    multi_modifier = CompositeModifier([date_modifier, country_modifier, color_modifier])
...
def example_6_dataset_simulation():
    ...
    print_section("Example 6: Dataset-Level Spurious Correlation Simulation using spurious_transform")
